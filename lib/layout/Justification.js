    ...
    Justification.prototype.findBreaks = function(node, index, tolerance) {
        ...
        while(activeNode) {
            ...
            for (var j = index; j < this.nodes.length; j += 1) {
                ...
                } else if (this.nodes[j].isBox || (this.nodes[j].isPenalty && this.nodes[j].penalty === -Justification.INFINITY && j > index)) {
                    break;
                }
            }
    ...
    Justification.prototype.makeLines = function(tolerance) {
        ...
        that.nodes.forEach(function(node, index) {
            if (node.isBox) {
                ...
                if (index > 0 && that.nodes[index - 1].isBox) {
        ...
        if (that.activeNodes.getSize() !== 0) {
            ...
            positions.push([that.nodes.length, 0, this.getLineWidth(breakPoint.line + 1)]);
            ...
            for (var i = 0; i < positions.length; i += 1) {
                ...
                for (var j = lineStart; j < that.nodes.length; j += 1) {
					...
					if (that.nodes[j].isBox || (that.nodes[j].isPenalty && that.nodes[j].penalty === -Justification.INFINITY)) {
						lineStart = j;
						break;
					}
                }
