...
goog.scope(function() {
    ...
    Paragraph.prototype.addBox = function(node, breakClass) {
        var position = new DomPosition(node.parentNode, node);
     ...
     * @param {!Element} node
     */
    Paragraph.prototype.addPenalty = function(node) {
        this.nodes.push(new Penalty(new DomPosition(node.parentNode, node), node.clientWidth, node.clientHeight, Justification.Demerits.HYPHEN, 1));
    };
                    ...
                    array.toArray(node.childNodes).forEach(function(box, index, boxes) {
                        if (index < boxes.length - 1) {
                            ...
                            if (index < boxes.length - 2) {
                                that.addPenalty(boxes[boxes.length - 1]);
                            }
                        }
                    });
                } else {
