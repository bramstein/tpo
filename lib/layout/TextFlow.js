...
goog.scope(function() {
     ...
     */
    TextFlow.prototype.flow = function(text) {
        var breaks = [],
            boxes = [],
            nodes = [];
        ...
        if (breaks.length) {
            ...
            for (var i = 0, blockIndex = 0; i < text.size; i += 1) {
                if (i > breaks[blockIndex][0]) {
                    ...
                    for (var j = i; j < text.size; j += 1) {
                        ...
                        if ((type === TextBlock.NodeType.BOX || type === TextBlock.NodeType.ELEMENT) || (type === TextBlock.NodeType.PENALTY && text.getPenalty(j) === -10000)) {
                            i = j;
                            break;
                        }
                        text.setX(j, -1);
                    ...
                    boxes.push(new TextBox(text, breaks[blockIndex][2], maxHeight, nodes));
                    nodes = [];
                    maxHeight = 0;
                ...
                if (type === TextBlock.NodeType.BOX) {
                    text.setX(i, x);
                    x += text.getWidth(i);
                    maxHeight = Math.max(maxHeight, text.getHeight(i));
                    nodes.push(i);
                } else if (type === TextBlock.NodeType.PENALTY && text.getPenalty(i) === 100 && i === breaks[blockIndex][0]) {
                    text.setX(i, x);
                    x += text.getWidth(i);
                    maxHeight = Math.max(maxHeight, text.getHeight(i));
                    nodes.push(i);
            ...
            debug.error('Could not set TextBlock', text);
        }
        return boxes;
    };
    ...
    TextFlow.prototype.computeRatio = function(text, index, breakPoint, runningTotal, blockIndex) {
        ...
        if (text.getType(index) === TextBlock.NodeType.PENALTY) {
    ...
    TextFlow.prototype.findBreaks = function(text, index, activeNodes, runningTotal, tolerance) {
        ...
        while(activeNode) {
            ...
            while(activeNode) {
                ...
                if (ratio < -1 || (text.getType(index) === TextBlock.NodeType.PENALTY && text.getPenalty(index) === -10000)) {
                ...
                if (-1 <= ratio && ratio <= tolerance) {
                    ...
                    // Positive penalty
                    if (text.getType(index) === TextBlock.NodeType.PENALTY && text.getPenalty(index) >= 0) {
                        demerits = Math.pow(10 + badness + text.getPenalty(index), 2);
                    // Negative penalty but not a forced break
                    } else if (text.getType(index) === TextBlock.NodeType.PENALTY && text.getPenalty(index) !== -10000) {
                        demerits = Math.pow(10 + badness - text.getPenalty(index), 2);
                    // All other cases
                    } else {
                    ...
                    if (text.getType(index) === TextBlock.NodeType.PENALTY && text.getType(activeNode.value.position) === TextBlock.NodeType.PENALTY) {
            ...
            for (var j = index; j < text.size; j += 1) {
                ...
                } else if ((type === TextBlock.NodeType.BOX || type === TextBlock.NodeType.ELEMENT) || (type === TextBlock.NodeType.PENALTY && text.getPenalty(j) === -10000 && j > index)) {
                    break;
                }
            }
    ...
    TextFlow.prototype.tryLayout = function(text, tolerance) {
        ...
        for (var i = 0; i < text.size; i += 1) {
            ...
            if (type === TextBlock.NodeType.BOX || type === TextBlock.NodeType.ELEMENT) {
                ...
                if (i > 0 && (text.getType(i - 1) === TextBlock.NodeType.BOX || text.getType(i - 1) === TextBlock.NodeType.ELEMENT)) {
            ...
            } else if (type === TextBlock.NodeType.PENALTY && text.getPenalty(i) !== 10000) {
                this.findBreaks(text, i, activeNodes, runningTotal, tolerance);
            }
        }
