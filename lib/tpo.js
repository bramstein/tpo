...
goog.require('tpo.layout.TextLayout');
...
goog.scope(function() {
    ...
    /*
        var tolerances = [2, 3, 6, 12, 24],
            fragment = window.document.createDocumentFragment();>>>>>>> Stashed changes
        ...
        for (var l = 0; l < paragraphs.length; l += 1) {
            var paragraph = paragraphs[l],
                lines = [];

            for (var i = 0; i < tolerances.length; i += 1) {
                lines = paragraph.makeLines(tolerances[i]);
            ...
            if (lines.length === 0) {
                ...
                for(var j = 0; j < lines.length; j += 1) {
                    ...
                    var x = 0,
                        maxHeight = paragraph.lineHeight,
                        node = null;

                    for(var index = 0; index < line.nodes.length; index += 1) {
                        node = line.nodes[index];

                        if (node.isBox || (node.isPenalty && node.penalty === Justification.Demerits.HYPHEN && index === line.nodes.length - 1)) {
            ...
            paragraph.element.style.height = y + 'px';
