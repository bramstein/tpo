...
goog.scope(function() {
    ...
    BreakAction.prototype.write = function(position, token) {
        ...
        if (this.previousBreakClass) {
            if (token.breakClass === BreakClass.SP) {
                this.previousToken.breakAction = BreakAction.Type.PROHIBITED;
            } else {
                breakAction = BreakAction.PairTable[this.previousBreakClass][token.breakClass];

                if (breakAction === BreakAction.Type.INDIRECT) {
                    if (this.previousToken.breakClass === BreakClass.SP) {
                        this.previousToken.breakAction = BreakAction.Type.INDIRECT;
                    } else {
                        this.previousToken.breakAction = BreakAction.Type.PROHIBITED;
                    }
                    this.previousBreakClass = token.breakClass;
            ...
            if (token.breakClass === BreakClass.SP) {
                token.breakClass = BreakClass.WJ;
            }
            this.previousBreakClass = token.breakClass;
        }
