...
goog.scope(function() {
    ...
    /**
     * @param {Array.<tpo.text.Token>} tokens
     * @return {Array.<tpo.text.BreakAction.Type>}
     */
    BreakAction.prototype.find = function(tokens) {
        ...
        for (var i = 1; i < tokens.length; i += 1) {
            if (tokenClass === Token.Class.BK) {
                breaks[i - 1] = BreakAction.Type.EXPLICIT;
            ...
            if (tokens[i].tokenClass === Token.Class.BK ||
                ...
                breaks[i - 1] = BreakAction.Type.PROHIBITED;
                tokenClass = Token.Class.BK;
                continue;
            }
            ...
            if (breakAction === BreakAction.Type.INDIRECT) {
                if (tokens[i - 1].tokenClass === Token.Class.SP) {
                    breaks[i - 1] = BreakAction.Type.INDIRECT;
                } else {
                    breaks[i - 1] = BreakAction.Type.PROHIBITED;
                }
                ...
                breaks[i - 1] = BreakAction.Type.PROHIBITED;
                if (tokens[i - 1].tokenClass === Token.Class.SP) {
                    breaks[i - 1] = BreakAction.Type.COMBINING_INDIRECT;
                } else {
                    continue;
                }
            } else if (breakAction === BreakAction.Type.COMBINING_PROHIBITED) {
                breaks[i - 1] = BreakAction.Type.COMBINING_PROHIBITED;
                if (tokens[i - 1].tokenClass !== Token.Class.SP) {
                    continue;
                }
            }
